...
const CATEGORY_LEGEND = [
  { label: 'P', text: 'Penalty' },
  { label: 'C', text: 'Corner' },
  { label: 'FK', text: 'Vrije trap' },
];
...
export default function SetPiecesTab() {
  ...
  const fetchSetPieces = useCallback(async () => {
    ...
    try {
      ...
      setError('Kon set piece-gegevens niet laden, probeer opnieuw.');
    } finally {
      setLoading(false);
    }
  }, []);
    ...
    <>
      ...
      {loading && (
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: '#C9B8E0', fontSize: '13px', marginBottom: '12px' }}>
          <Loader2 size={16} className="fdr-spin" /> Set piece-gegevens laden...
        </div>
      )}
      ...
      {!loading && !error && entries.length === 0 && (
        <div style={{
          ...
          <p style={{ color: '#C9B8E0', fontSize: '13px', margin: 0 }}>
            Nog geen set piece-gegevens beschikbaar — kom binnenkort terug.
          </p>
        </div>
      )}
